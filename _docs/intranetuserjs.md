...
Tarpeellisuus: Ei enää tarpeellinen versiossa 25.05, koska othername-kentän tieto siirtyi yhteisössä uutena lisättyyn preferred name / kutsumanimi -kenttään.
Versio: 24.05
...
Vanha versio, nykyään asiakasmääreillä on jokaisella oma tunniste.
...
Tarpeellisuus: Ei tarpeellinen versiossa 25.05, koska othername-kentän tieto on siirretty uuteen preferred_name-kenttään<br />
Versio: 23.11
...
```
/// ALKU ///
// Siirretään varaukset alemmas asiakkaan lainaus- ja tiedot-sivuilla ja luodaan niille pudotusvalikkopiilotus
$( document ).ready(function() {
  if ( window.location.href.indexOf("members/moremember.pl") > -1 || window.location.href.indexOf("circ/circulation.pl") > -1 ){
    ...
    $( '#holdswaiting > h4:nth-of-type(1)' ).attr("data-bs-toggle","collapse");
    $( '#holdswaiting > h4:nth-of-type(1)' ).attr("data-bs-target","#collapseOmat");
    ...
    $( '#holdswaiting > h4:nth-of-type(2)' ).attr("data-bs-toggle","collapse");
    $( '#holdswaiting > h4:nth-of-type(2)' ).attr("data-bs-target","#collapseMuut");
...
Tarpeellisuus: Ei enää tarpeellinen, koska toiminto on tullut osaksi Kohan omaa toiminnallisuutta versiossa 25.05<br />
Versio: 23.11
...
Tarpeellisuus: Ei tarpeellinen, siirretty osaksi IntranetJS: ChangesToSearchPage -liitännäistä<br />
Versio: 23.11
...
Tällä skriptillä saa siirrettyä niteen muokkauksessa nidelistauksen muokattavana olevan niten tietojen alapuolelle. Ei välttämättä enää tarvetta, koska niteen muokkaus ohjautuu suoraan muokattavan niteen lomakkeen kohdalle. Saattaa olla kuitenkin selkeämpi.
...
//Cataloging > Z39.50/SRU pop-up
 //BEGIN Resize Z39.50 window
  if (document.location.href.indexOf('z3950_search.pl')>-1) window.resizeTo((screen.width * 0.7), (screen.height * 0.8)), window.moveTo(0, 0);
...
### Piilota IntranetJS-pluginit Ylläpito-sivulta työkaluliitännäisten listasta

Rimpsulla voi piilottaa Ylläpito-sivulta linkit IntranetJS-liitännäisiin. Ne näkyvät, kun menee Liitännäiset-sivulle.

Tarpeellisuus: Suositeltava<br />
Versio 25.05

```
/// Piilota IntranetJS-pluginit Kohan ylläpito-näkymästä liitännäisten listasta 
$(document).ready(function () {
  if (window.location.href.indexOf("admin/admin-home.pl") > -1) {
    $('.plugin_name').closest('a').parent('dt').next('dd').hide();
    $('.plugin_name').closest('a').parent('dt').hide();
}});
```
...
```
$(document).ready(function() {
  if (typeof dataTablesDefaults !== 'undefined') {
  	dataTablesDefaults.fixedHeader = false;
  }
}); 
```
...
$(document).ready(function() {
  if ( "body#pat_member-flags.pat" ){
    ...
    var laskutus = [ "plugins_tool", "flag-10", "borrowers_edit_borrowers", "borrowers_view_borrower_infos_from_any_libraries", "editcatalogue_edit_items" ];
    var automaatti = [ "flag-1" ];
    var esihenkilo = [ "flag-17" ];
    var paakayttaja = [ "flag-0" ];
      
    var oikeudetNappi = "<div class='btn-group'><button class='btn btn-default dropdown-toggle' data-bs-toggle='dropdown'><i class='fa fa-plus'></i> Lisää käyttöoikeuksia <span class='caret'></span></button><ul class='dropdown-menu'><li><a href=# id='asiakaspalvelu'>Asiakaspalvelu</a></li><li><a href=# id='kevyttunnus1'>Kevyttunnus 1</a></li><li><a href=# id='kevyttunnus2'>Kevyttunnus 2</a></li><li><a href=# id='kuvailu'>Kuvailu</a></li><li><a href=# id='hankintaJaKausijulkaisut'>Hankinta ja kausijulkaisut</a></li><li><a href=# id='erapaivakalenteri'>Eräpäiväkalenteri</a></li><li><a href=# id='laskutus'>Laskutus</a></li><li><a href=# id='automaatti'>Automaatti</a></li><li><a href=# id='esihenkilo'>Esihenkilö</a></li><li><a href=# id='paakayttaja'>Pääkäyttäjä</a></li></ul></div>";
    ...
    $('#esihenkilo').click( function(){
      for (i=0; i<esihenkilo.length; i++){
        if ( $('#' +esihenkilo[i]).prop('checked') == false){
          $('#' +esihenkilo[i]).click();
        }
      }
    });
...
Tarpeellisuus: **Korvattu** IntranetUserJS: Istunnon vanhentumisilmoitus -liitännäisellä <br />
Versio: 24.05
...
### Yläpalkin linkkien siirto oikealle

Testattu Vaskin nextillä toimivaksi. Vaatii IntranetNav-asetuksessa oleviin elementteihin intranetnav-luokan lisäämisen.

```
/// ALKU ///
/* IntranetNav linkkien siirtäminen samaan kohtaan yläpalkkia kuin aiemmissa versioissa */
$( document ).ready(function() {
  var lisatietoja = $('#toplevelmenu li:last').prev();
  var siirto = $( '.intranetnav' ).detach();
  $(lisatietoja).after(siirto);
});
/// LOPPU ///
```

Esimerkki IntranetNav-linkistä. Intranetnav-luokka tulee laittaa vain ylimmän tason li-elementteihin.

```
<li class="nav-item intranetnav"><a class="nav-link" href="https://vaski.finna.fi" target="_finna"><span class="nav-link-icon"><i class="fa-solid fa-arrow-up-right-from-square" title="Finna"></i></span><span class="nav-link-text">Finna</span></a></li>
```
```
```
