...
Turussa tehty versio vanhentuneiden noutamattomien varausten raportista. Raportti näyttää joko varaustunnisteen tai noutohyllyn riippuen siitä, löytyykö varaukselta noutohylly-tietoa. Raportin tulokset järjestetään varaustunniste/noutohylly-sarakkeen mukaan.

```
SELECT IF(olr.hold_pickup_shelf_id IS NOT NULL, CONCAT('<a href=\"/cgi-bin/koha/members/moremember.pl?borrowernumber=',olr.borrowernumber,'\">',(SELECT hps.shelf_name FROM hold_pickup_shelves hps WHERE hps.hold_pickup_shelf_id = olr.hold_pickup_shelf_id),'</a>'), CONCAT('<a href=\"/cgi-bin/koha/members/moremember.pl?borrowernumber=',ba.borrowernumber,'\">',ba.attribute,'</a>')) AS "Hyllypaikka/Varaustunnus",
...
FROM (SELECT IF(o.borrowernumber=28, REGEXP_SUBSTR(SUBSTRING(al.info,(LOCATE("'borrowernumber' =>", al.info)+20), 7), '^[0-9]+'), o.borrowernumber) as borrowernumber,
	o.reserve_id,
    o.biblionumber,
    o.itemnumber,
    o.cancellationdate,
	...
	FROM old_reserves o
	LEFT JOIN (SELECT * FROM action_logs WHERE module='HOLDS' AND action='CANCEL') al ON al.object = o.reserve_id) olr 
LEFT JOIN (SELECT * FROM borrower_attributes WHERE code='HOLDID') ba using(borrowernumber)
LEFT JOIN biblio using(biblionumber)
LEFT JOIN items using(itemnumber)
LEFT JOIN biblioitems using(biblioitemnumber)
LEFT JOIN hold_pickup_shelves hps using(hold_pickup_shelf_id)
WHERE olr.cancellationdate > DATE(items.datelastseen)
    ...
    AND olr.itemnumber NOT IN (SELECT itemnumber FROM issues WHERE issues.borrowernumber=olr.borrowernumber AND issues.itemnumber=olr.itemnumber)
ORDER BY
CASE WHEN olr.hold_pickup_shelf_id IS NOT NULL THEN SUBSTR(hps.shelf_name,1,REGEXP_INSTR(hps.shelf_name, '[0-9]')-1) END,
CASE WHEN olr.hold_pickup_shelf_id IS NOT NULL THEN CAST(SUBSTR(hps.shelf_name, REGEXP_INSTR(hps.shelf_name, '[0-9]')) AS INT) END,
...
CASE WHEN ba.attribute REGEXP '^[0-9]' THEN cast(REPLACE(ba.attribute, ' ', '') as int) END ASC
```
